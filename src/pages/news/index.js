...
const News = ({ toggleChat = () => {} }) => {
  ...
  const initializeChatbot = () => {
    setLoading(false);
    setChatHistory([
      { role: 'model', parts: ['Hi, I am Incity NewsBot. Ask me about the latest news!'] },
      { role: 'model', parts: ['You can ask me about recent headlines or news related to specific topics.'] },
      { role: 'model', parts: ['For example, you can type "latest technology news" or "news about sports" to get updates.'] }
    ]);
  };
              ...
              {message.role === 'model' ? (
                message.parts.map((item, idx) => (
                  <div key={idx} className="mb-4">
                    <div className="font-bold text-lg">{item.title || 'Incity NewsBot'}</div>
                    <div className="mt-1 mb-2">{item.description || item}</div>
                  </div>
                ))
              ) : (
                `${message.role === 'user' ? 'You' : 'Incity NewsBot'}: ${message.parts}`
              )}
            </div>
          ))}
